...
cdef class NvRenderTexture:
    ...
    @classmethod
    def from_rl_render_texture(cls, rl_render_texture: rl.RenderTexture): 
    ...
    def __init__(self, size: NvVector2):
        self.render_texture = rl.load_render_texture(int(size.x), int(size.y))
        self.size = size
    ...
    cpdef void blit(self, nv_texture: NvRenderTexture, dest: Annotations.dest_like, blend_mode: int = rl.BlendMode.BLEND_ALPHA, flip: bool = True, color: Annotations.rgba_color = Color.White):
        assert self.loaded, "Render texture not loaded"
        rl.begin_texture_mode(self.render_texture) # type: ignore
        self.fast_blit(nv_texture, dest, blend_mode, flip, color)
        rl.end_texture_mode()
    
    cpdef void fast_blit(self, nv_texture: NvRenderTexture, dest: Annotations.dest_like, blend_mode: int = rl.BlendMode.BLEND_ALPHA, flip: bool = True, color: Annotations.rgba_color = Color.White):
    ...
    cpdef void kill(self):
        assert self.loaded, "Render texture not loaded"
        rl.unload_render_texture(self.render_texture) # type: ignore
    ...
    def copy(self, flip: bool = True):
        ...
        new_nvtexture.blit(self, (0, 0), rl.BlendMode.BLEND_ALPHA, flip)
        return new_nvtexture
    
    def __dealloc__(self):
        try:
            if hasattr(self, 'render_texture') and self.loaded and rl.is_window_ready():
                rl.unload_render_texture(self.render_texture) # type: ignore
