...
class Grid(LayoutType):
    ...
    def _regenerate_coordinates(self):
        ...
        for i in range(len(self.items)):
            ...
            cw, ch = (self._rsize.x / self.column, self._rsize.y / self.row) if self.menu else (self.relx(self.cell_width), self.rely(self.cell_height))
            coordinates = NvVector2(self.coordinates.x + self._rsize_marg.x + x * cw + (cw - self.relx(item.size.x)) / 2 ,
                                    self.coordinates.y + self._rsize_marg.y + y * ch + (ch - self.rely(item.size.y)) / 2)
