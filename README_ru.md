...
<a href="https://golembebrov.github.io/nevu-docs/">Nevu-UI документация</a>

**Nevu UI** - это GUI библиотека для простого создания интерфейсов на `Python`, которая предоставляет набор готовых настраиваемых компонентов и удобных инструментов для создания интерфейсов в играх и приложениях.

### Основные функции:
*   **Макеты:** Контейнеры, которые автоматически располагают элементы внутри себя, например `Grid`, `ScrollableColumn`.
...
*   **Разные режимы создания интерфейсов** Поддержка декларативного и императивного создания с возможностью их комбинировать.
*   **Несколько бекендов**: Поддержка `Raylib` и `Pygame`
*   **Легкое встраивание**: Возможность встраивания `nevu-ui` в готовую игру или приложение используя `InitializedWindow`
---
...
### Декларативность
  ...
  *   **Создание интерфейсов:**
      ...
      app.run()
      ```
*   **Система размеров:** Дает возможность использовать относительные величины для указания первоначальной высоты/ширины объекта вместо пикселей.
    ...
    *   Префикс `c`: можно поставить в начало любой величины кроме auto(например `cvh`), он означает, что будет браться текущий размер окна/макета, без префикса будет браться оригинальный.
    *   `auto` автоматическое определение на основе контента.
### Анимации:
  **25+ разных анимаций**
...
### В `Nevu UI` могут встречаться баги так как он еще в beta версии.
...
### **Создал:** GolemBebrov / ГолемБебров
