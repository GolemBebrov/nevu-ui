...
from nevu_ui.fast import NvVector2, NvRect, NvRenderTexture, Cache, NvParam
...
class MenuRendererProxy:
    ...
    @property
    def _hover_state(self): return HoverState.NotHovered
    @property
...
class MenuLayoutProxy:
    def __init__(self, menu):
        ...
        self.booted = True
    @property
    def original_size(self): return self.menu._window.size
    def __getattr__(self, name): 
        if name == "original_size": return self.menu._window.size
        return getattr(self.menu, name)


class Menu:
    def _initial_resize(self):
        if self._layouts_cook_time:
            if abs(self._layouts_cook_time - self._layouts_cook_time_max) > self._layouts_cook_time_delay:
                self.add_next_frame_action(lambda: (self._resize_with_ratio(self._resize_ratio), self._initial_resize()))
            else:
                self.add_next_frame_action(self._initial_resize)
            self._layouts_cook_time -= 1
    __slots__ = ('_absolute_coordinates', '_tuple_absolute_coordinates', '_draw_borders', '_layouts_cook_time', '_layouts_cook_time_max', '_layouts_cook_time_delay',
                 "_first_update_functions", "_next_frame_functions", '_draw_content', 'override_color', 'alt', '_window', 'window_surface', 'cache', '_style', '_subtheme_role', 'original_size', '_renderer_proxy', '_renderer', '_layout_proxy', '_convert_item_coord', '_parse_fillx', '_parse_gcx', '_parse_vx', '_percent_helper', '_read_item_coords', '_resize_ratio', '_size', '_rel_size', 'coordinates', '_layout', '_changed', '_relative_placed', '_bg_transparent', '_relative_x', '_relative_y', 'enabled', 'visible', '_main_draw', '_subtheme_border', '_subtheme_content', 'first_window_size', 'first_coordinates', '_opened_sub_menu', '_args_menus_to_draw', '_surface')
    def __init__(self, window: Window | None, size: list | tuple | NvVector2, style: Style = default_style, alt: bool = False, layout = None, _draw_borders = True, _draw_content = True, override_color = None): 
        ...
        self._layouts_cook_time_delay = 1
        ...
        if self._window.renderer_type.raylib: self._renderer = RaylibRenderer(self._renderer_proxy) 
    ...
    def convert_to_sdl2_texture(self, surface = None):
        ...
        if not self._window.renderer_type.sdl: raise ValueError("convert_to_sdl2_texture is supported only in SDL backend! UWO")
    ...
    def _generate_background(self):
        ...
        return self.convert_to_sdl2_texture(base_func) if nevu_state.window.renderer_type.sdl else base_func
    ...
    def _resize(self, size: NvVector2):
        self._resize_ratio = NvVector2.from_xy(size[0] / self.first_window_size[0], size[1] / self.first_window_size[1])
        self._resize_base()
    ...
    def _resize_base(self):
        ...
        if self._relative_placed:
            ...
            target_pos = window_size * (NvVector2.from_xy(self._relative_x, self._relative_y) / 100)
    ...
    def layout(self, layout: LayoutType):
        ...
        layout.coordinates = NvVector2.from_xy(self.size[0]/2 - layout.size[0]/2, self.size[1]/2 - layout.size[1]/2)
    ...
    def set_coordinates_relative(self, percent_x: int, percent_y: int):
        if self._window is None: raise ValueError("Window is not initialized!")
        self.coordinates = NvVector2.from_xy((self._window.size[0]-self._window._crop_width_offset)/100*percent_x-self.size[0]/2,
    ...
    def get_nvrect(self): return NvRect(NvVector2.from_xy(0, 0), self._rel_size)
