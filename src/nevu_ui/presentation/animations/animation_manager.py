...
from nevu_ui.presentation.animations.animation_base import (
    Animation,
    AnimationQueue,
    ColorAnimation,
    FloatAnimation,
...
class AnimationManager:
    __slots__ = [
        "_basic_set_of_animations",
        "_basic_animation_type_requirements",
        "_warn",
        "_start_animations",
        "_continuous_animations",
        "_transition_animations",
        "transition_animation_easing",
        "transition_time",
        "state",
        "_running",
    ]
        ...
        match self.state:
            case State.Start:
                ...
                for animation in current_animations.values():
                    ...
                    animation.update()
                if all(
                    animation is None or animation.ended
                    for animation in current_animations.values()
                ):
    ...
    def _add_animation_template(
        self, animations_dict, animation_key: AnimationType | str, animation: Animation
    ):
        if animation_key in animations_dict and self._warn:
            print(
                f"Warning: An animation of type {animation_key} already exists. It will be overwritten."
            )
        if animation_key in self._basic_animation_type_requirements and not isinstance(
            animation,
            (self._basic_animation_type_requirements[animation_key], AnimationQueue),
        ):
            raise ValueError(
                f"Animation of type {animation_key} must be of type {self._basic_animation_type_requirements[animation_key].__name__}"
            )
        animations_dict[animation_key] = copy.copy(animation)
        animations_dict[animation_key].reset()

    def add_start_animation(
        self, animation_key: AnimationType | str, animation: Animation
    ):
        self._add_animation_template(self._start_animations, animation_key, animation)

    def add_continuous_animation(
        self, animation_key: AnimationType | str, animation: Animation
    ):
        self._add_animation_template(
            self._continuous_animations, animation_key, animation
        )
    ...
    def _start_transition_animations(self):
        for anim_type, cont_anim in self._continuous_animations.items():
            ...
            transition_time = (
                cont_anim.max_time / 2
                if self.transition_time is None
                else self.transition_time
            )
            ...
            transition_anim = anim_class(
                start_value,
                end_value,
                transition_time,
                self.transition_animation_easing,
            )
