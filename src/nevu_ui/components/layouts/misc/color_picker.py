...
class ColorPicker(Grid):
    ...
    def _regenerate_items(self):
        if self.amount_of_colors <= 0:
            raise ValueError("Amount of colors must be greater than 0")
        if self.item_size.x <= 0 or self.item_size.y <= 0:
        ...
        self.size = size + NvVector2(
            self.margin * (self.amount_of_colors - 1),
            self.margin * (self._widget_line - 1),
        )
        ...
        for i in range(self.amount_of_colors):
            self.add_item(
                Input(
                    ...
                    on_change_function=self._return_colors,
                    max_characters=3,
                ),
                i + 1,
                self._widget_line,
    ...
    def on_change_function(self):
        return self.get_param_strict("on_change_function").value

    @property
    def item_size(self):
        return self.get_param_strict("item_size").value

    @property
    def margin(self):
        return self.get_param_strict("margin").value

    @property
    def input_style(self):
        return self.get_param_strict("input_style").value

    @property
    def label_style(self):
        return self.get_param_strict("label_style").value

    @property
    def amount_of_colors(self):
        return self._amount_of_colors
    ...
    def _return_colors(self, *args):
        if self.on_change_function:
            self.on_change_function(self.get_color())

    def get_color(self) -> tuple:
        color = []
        color.extend(
            int(item.text) if item.text != "" else 0
            for item in self.items
