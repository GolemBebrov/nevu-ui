...
class Animation:
    __slots__ = ["max_time", "curr_time", "start", "end", "ended", "current_value", "easing_func", "check_errors"]
...
class Vector2Animation(Animation):
    __slots__ = ()
    def __init__(self, start: NvVector2 | tuple | list, end: NvVector2 | tuple | list, time: int | float = 1, easing_func: Callable | None = None, check_errors: bool = False): 
        super().__init__(NvVector2(start), NvVector2(end), time, easing_func, check_errors)
    def _apply_easing(self, eased_value):
        if isinstance(eased_value, tuple):
            ...
            self.current_value = NvVector2(*(self.start[i] + (self.end[i] - self.start[i]) * eased_value for i in range(2)))
    ...
    def _apply_easing(self, eased_value):
        ...
        if isinstance(eased_value, tuple):
            ...
            self.current_value = tuple(
                self.start[i] + (self.end[i] - self.start[i]) * progress + (offset[i] if is_seq else offset)
                for i in range(length)
            )
        else:
            self.current_value = tuple(self.start[i] + (self.end[i] - self.start[i]) * eased_value for i in range(length))
...
class FloatAnimation(Animation):
    ...
    def _apply_easing(self, eased_value):
        if isinstance(eased_value, tuple):
            progress, offset = eased_value
            self.current_value = self.start + (self.end - self.start) * progress + offset
        else:
