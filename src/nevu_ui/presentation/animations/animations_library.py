...
def ease_in_bounce(value: float) -> float:
...
# === Sine ===
def ease_in_sine(value: float) -> float:
...
# === Quart ===
def ease_in_quart(value: float) -> float:
    return value**4
...
# === Expo ===
def ease_in_expo(value: float) -> float:
    return 0.0 if value == 0 else pow(2, 10 * value - 10)


def ease_out_expo(value: float) -> float:
    return 1.0 if value == 1 else 1 - pow(2, -10 * value)
...
# === Circ ===
def ease_in_circ(value: float) -> float:
    return 1 - math.sqrt(1 - pow(value, 2))


def ease_out_circ(value: float) -> float:
...
# === Power ===
def ease_in_power(power: float) -> Callable[[float], float]:
    return lambda value: pow(value, power)


def ease_out_power(power: float) -> Callable[[float], float]:
...
def smooth_shake_easing(
    amplitude: float = 8.0, frequency: float = 20.0, decay: float = 5.0
):
    def easing(progress: float):
        ...
        offset = NvVector2.from_xy(
            current_amp * math.cos(angle), current_amp * math.sin(angle * 1.3)
        )
        return progress, offset
