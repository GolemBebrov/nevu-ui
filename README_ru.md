...
*   **Встроенные анимации:** Оживите ваш интерфейс с помощью готовых анимаций движения, прозрачности и т.д.
    * Есть **2** типа анимаций: 
        ...
        *   **Безконечная** - Производит бесконечную анимацию заданную в `animation_manager`.
    * Пример использования:
       * ```widget.animation_manager.add_start_animation(ui.animations.EaseOut(...))```

Система констант(Constant Engine):

*   `ConstantEngine` это удобный инструмент встроенный во все макеты и виджеты, он позволяет:
    * Декларативно добавлять переменные в `__init__` обьекта
    * Проверять тип переменной во время инициализации и после
    * Переменная не будет видна в подсказках к `__init__` если он не был добавлен в `TypedDict` класса
    * Переменная не будет видна **ВО ВСЕХ** подсказках если вы не укажите название и тип в начале класса
*   **Примеры:**
    *   ```python 
        import nevu_ui as ui
        from typing import Unpack, NotRequired

        #Создаем TypedDict с переменными (необязательно)
        class MyWidgetKwargs(ui.WidgetKwargs):
            my_var: NotRequired[int | float] 

        class MyWidget(ui.Widget):
            #Создаем typehint для переменной (необязательно но рекомендуется)
            my_var: int | float
            def __init__(self, size: NvVector2 | list, style: Style = default_style, **constant_kwargs: Unpack[MyWidgetKwargs]):
                super().__init__(size, style, **constant_kwargs)
            
            #Переопределяем функцию для добавления констант (обязательно)
            def _add_constants(self):
                super()._add_constants()

                #Добавляем константу (обязательно)
                self._add_constant('my_var', int | float)

                #Можно еще добавить ссылку на константу
                #self._add_constant_link('my_var', 'my_var_new_name')

                #Также можно заблокировать константу при необходимости
