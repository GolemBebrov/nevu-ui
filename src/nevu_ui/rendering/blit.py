...
class AlphaBlit:
    @staticmethod
    def blit(dest_surf: "Surface", source_surf: "Surface", dest_pos: tuple[int, int]):
        x, y = dest_pos
        width, height = source_surf.get_size()
        roi_rect = md.pygame.Rect(x, y, width, height)
        ...
        try:
            ...
            source_alpha_view = md.pygame.surfarray.pixels_alpha(source_surf)[src_slice_x, src_slice_y]
            dest_alpha_view = md.pygame.surfarray.pixels_alpha(dest_surf)[dest_slice_x, dest_slice_y]
            
            np.copyto(dest_alpha_view, source_alpha_view)

        except ValueError:
            clipped_source_rect = md.pygame.Rect(src_x_offset, src_y_offset, roi_rect_clipped.width, roi_rect_clipped.height)
            dest_surf.blit(source_surf.subsurface(clipped_source_rect), roi_rect_clipped.topleft, special_flags=md.pygame.BLEND_RGBA_MULT)

class ReverseAlphaBlit:
    @staticmethod
    def blit(dest_surf: "Surface", source_surf: "Surface", source_pos: tuple[int, int]):
        dest_width, dest_height = dest_surf.get_size()
        src_x, src_y = source_pos

        source_read_rect = md.pygame.Rect(src_x, src_y, dest_width, dest_height)
        ...
        dest_write_rect = md.pygame.Rect(
            dest_x_offset,
            dest_y_offset,
        ...
        try:
            ...
            dest_surf.blit(clipped_source_subsurf, dest_write_rect.topleft, special_flags=md.pygame.BLEND_RGBA_MULT)