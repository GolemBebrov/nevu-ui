...
class Manager:
    __slots__ = ("_window", "running", "force_quit", "_background", "_fps", "_static_run", "init", "_started", "menus")

    def __new__(cls, *args, **kwargs):
        global manager_created
        if manager_created: raise RuntimeError("Manager is already created!")
    ...
    @property
    def static_run(self): return self._static_run
    @static_run.setter
    ...
    def background(self, color):
        if self._started and self.static_run:
            raise RuntimeError("You can't change background after starting static main loop!") 
    ...
    def run(self): 
        if self.static_run: self.__main_loop_opt()
        else: self.__main_loop_base()
