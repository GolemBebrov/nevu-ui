...
class GradientPygame:
    def __init__(self, colors: list[Annotations.rgb_like_color], type: GradientType = GradientType.Linear, direction: GradientConfig = LinearSide.Right, transparency = None):
    ...
    def _blit_numpy_gradient(self, surface, progress):
        ...
        colors_array = np.array(colors_only, dtype=np.float32)
    ...
    def _validate_colors(self, colors):
        ...
        validated_colors =[]
        for item in colors:
            if isinstance(item, (tuple, list)) and len(item) == 2 and isinstance(item[1], (int, float)) and (isinstance(item[0], str) or (isinstance(item[0], (tuple, list)) and len(item[0]) in (3, 4))):
            ...
            if isinstance(color_val, str):
                try:
                    ...
                    if isinstance(color_tuple, tuple) and len(color_tuple) in (3, 4): 
                        ...
                        raise ValueError(f"Invalid color {color_tuple} with name {color_val}.")
                except (AttributeError, ValueError) as e:
                    raise ValueError(f"Unsupported color name: '{color_val}'.") from e
            elif isinstance(color_val, (tuple, list)) and len(color_val) in (3, 4) and all(isinstance(c, int) and 0 <= c <= 255 for c in color_val):
                validated_colors.append((tuple(color_val), weight))
            else:
                raise ValueError("Each color must be a tuple of 3 or 4 integers (RGB), a valid color name, or a tuple of (color, weight).")
